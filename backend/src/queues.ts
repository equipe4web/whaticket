      ...
      if(status === true){
        ...
        if(dias <= -3){
          ...
          if (existingInvoice) {
            ...
            // Atualizar cobrança no Asaas se existir
            try {
              const invoice = await Invoices.findByPk(openInvoices[0].id);
              if (invoice && invoice.asaasPaymentId && c.cpfCnpj) {
                // Cancelar cobrança anterior e criar nova com nova data
                await AsaasBillingService.cancelPayment(invoice.asaasPaymentId, c.id);
                await AsaasBillingService.createPayment(c, invoice, "PIX");
                logger.info(`Cobrança Asaas atualizada para fatura ID: ${invoice.id}`);
              }
            } catch (asaasError) {
              logger.warn(`Erro ao atualizar cobrança no Asaas para empresa ${c.id}:`, asaasError);
            }
            ...
            // Criar cobrança no Asaas se empresa tem CPF/CNPJ
            if (c.cpfCnpj) {
              try {
                const newInvoice = await Invoices.findByPk(invoiceInsert[0]);
                if (newInvoice) {
                  await AsaasBillingService.createPayment(c, newInvoice, "PIX");
                  logger.info(`Cobrança Asaas criada para fatura ID: ${newInvoice.id}`);
                }
              } catch (asaasError) {
                logger.warn(`Erro ao criar cobrança no Asaas para empresa ${c.id}:`, asaasError);
              }
            }
          }
